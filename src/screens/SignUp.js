...
import { validateName, checkNormalData, checkName, checkEmail, checkMobile, checkPassword, } from '../Validation'
import SQLite from 'react-native-sqlite-storage';

SQLite.enablePromise(true); // Enable promises for easier async/await handling
...
export default function SignUp(props) {
  ...
  const [userNameError, setUserNameError] = useState({ status: false, string: '' });
  const [terms, setTermsError] = useState({ status: false, string: '' });
  ...
  const insertUser = async (db, name, email, month,phone) => {
    try {
      const insertQuery = `INSERT INTO users (name,email,month,phone) VALUES (?,?,?,?)`;
      await db.executeSql(insertQuery, [name, email, month,phone]);
    } catch (error) {
  ...
  const createAccount1 = () => {
    Keyboard.dismiss()
    if (!checkNormalData(userName, '').status && !checkEmail(email, '').status && !checkPassword(password, '').status) {
      setErrorState()
      createAccount()
    }
    else {
  ...
  const createAccount = () => {
    auth().createUserWithEmailAndPassword(email, password)
      .then(userCredential => {
        ...
        alert('You have successfully register your email')
        props.navigation.navigate('SignIn')
        let d = new Date();
        insertUser(db, userName, email, d.toString(),'')
      })
      .catch(error => {
        console.log(error);
        alert(error)
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps='handled'>
          ...
          <View style={style.mainview}>
            ...
            <InputText
              ...
              containerStyle={{ marginTop: 10, borderColor: userNameError.length > 0 ? '#87DC69' : '#EAEAEA' }}
              inputStyle={{ paddingLeft: 10 }}
              value={userName}
              onChangeText={value => { setUserName(value), setUserNameError(checkNormalData(value, 'Please enter email.')), nameValidate(value) }}
            />
            <ErrorView text={
              userNameError.text} show={userNameError.status}
            />
          ...
          <View style={style.belowcontainer}>
            <Text style={style.already}>Already Have An Account? <Text style={{ fontFamily: 'Poppins-SemiBold',color: colors.orange, }} onPress={() => props.navigation.push('SignIn')}>Sign In</Text></Text>
          </View>
        </ScrollView>
