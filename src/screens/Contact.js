...
import SQLite from 'react-native-sqlite-storage';
  ...
  export default function SignIn(props) {
    ...
    const updateUser = async (db, id, phone) => {
      try{
      const updateQuery = `UPDATE users SET phone = ? WHERE id = ?`;
      await db.executeSql(updateQuery, [phone, id]);
      alert('Phone Number update successfuly')
    ...
    const loginAccount = async () => {
      updateUser(db, currentId, phoneNumber)
        ...
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : null} style={{ flex: 1 }}>
          ...
          <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps='handled'>
            <View style={style.mainc}>
              ...
              <Text style={style.information}>{`Add Contact Information`}</Text>
            </View>
